...
/**
 * Compute tree-layout positions for org nodes so the canvas
...
/**
 * Seed default data into a new organization's tenant schema.
 * Reuses the shared seed data constants from seed-data.ts.
 *
 * @param prisma - Tenant-scoped Prisma client (already pointed at the tenant schema)
 */
export async function seedOrganization(prisma: PrismaClient): Promise<void> {
  ...
  if (!existingWorkspace) {
    await prisma.workspace.create({ data: DEFAULT_WORKSPACE });
    console.log("[SeedOrg] Created default workspace");
  }

  // 2) Create default tool permissions
  ...
  console.log(`[SeedOrg] Upserted ${TOOL_PERMISSION_SEED.length} tool permissions`);
  ...
  console.log(`[SeedOrg] Upserted ${AGENT_SEED.length} agents`);

  // 4) Create org hierarchy nodes (skip main agent -- it has no org position)
  ...
  if (!root) throw new Error("[SeedOrg] Seed data missing root agent");
  const rootId = agentsByName.get(root.name)?.id;
  if (!rootId) throw new Error("[SeedOrg] Root agent missing after upsert");

  const rootPos = treePositions.get(root.name) ?? { x: 0, y: 0 };
  const rootNode = await prisma.orgNode.upsert({
  ...
  while (pending.length > 0) {
    ...
    if (nextIndex === -1) {
      ...
      throw new Error(`[SeedOrg] Unresolved reporting chain: ${unresolved}`);
    }

    const agent = pending.splice(nextIndex, 1)[0]!;
    const agentId = agentsByName.get(agent.name)?.id;
    if (!agentId) throw new Error(`[SeedOrg] Agent missing after upsert: ${agent.name}`);

    const parentNodeId = agent.reportsTo ? nodeIdsByAgentName.get(agent.reportsTo) : rootNode.id;
    if (!parentNodeId) throw new Error(`[SeedOrg] Parent node missing for ${agent.name}`);
  ...
  console.log("[SeedOrg] Upserted org hierarchy with canvas positions");
}
