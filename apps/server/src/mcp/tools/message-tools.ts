...
export function createMessageTools(ctx: McpToolContext) {
  const { prisma, orgSlug, agentId, runtime } = ctx;
    ...
    tool(
      ...
      async (args) => {
        try {
          ...
          const formatted = messages.map((m: typeof messages[number]) => ({
            id: m.id,
    ...
    tool(
      ...
      async () => {
        try {
          ...
          return toolText(JSON.stringify(latestMessages.map((m: typeof latestMessages[number]) => ({
            threadId: m.threadId,
            from: m.sender?.name ?? "human",
            to: m.recipient.name,
            body: m.body,
